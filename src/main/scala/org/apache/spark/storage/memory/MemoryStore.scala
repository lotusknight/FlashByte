...
import org.apache.spark.storage.{BlockId, BlockInfoManager, StorageLevel, StreamBlockId, TransForPageRank}
...
//      val index = putIntoNative[T](vector.toArray)(classTag)
      ...
      memoryManager.synchronized {
        releaseUnrollMemoryForThisTask(MemoryMode.ON_HEAP, unrollMemoryUsedByThisBlock)
        success = memoryManager.acquireStorageMemory(blockId, index, MemoryMode.OFF_HEAP)
        assert(success, "transferring unroll memory to storage memory failed")
      }
