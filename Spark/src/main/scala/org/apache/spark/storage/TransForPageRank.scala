...
//    println("2st Direct ByteBuffer: " + Arrays.toString(snd_bb.array()))
  ...
  //  need to clean the stock_values and stock_index
    ...
    (TransForPageRank.index-1, _size)
  }

  def transback(idx: Integer): Array[Tuple2[String, Array[String]]] = {
    val entry = TransForPageRank.entries.synchronized{
      TransForPageRank.entries.get(idx)}
    entry match {
      case null =>
...
object TransForPageRank {
  ...
  private val entries = new LinkedHashMap[Integer, (ByteBuffer, ByteBuffer)](32, 0.75f, true)
